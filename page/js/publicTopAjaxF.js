    ...
    success:function(data){
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].userPhone == getCookie("userphone")){
                ...
                <li class='lis4'>"+data[i].proName+"</li>\
                <li class='lis5'>\
                    <p><span class='danjia price'>"+data[i].proColor+"</span></p>\
                </li>\
                <li class='lis6'>"+ data[i].proSize +"</li>\
            </ul>";
                $(".car_product_F").html(productPart);
                // shuju();
                big2();
            }
        }
...
function big2(){
    ...
    function allCheck(){
        //头部
        $("#selectAll01")[0].onclick=function(){
            if($("#selectAll01")[0].checked == true){
    ...
    function oneCheck(){
        ...
        for(let i=0;i<Check.length;i++){
            Check[i].onclick=function(){
                ...
                if(all == Check.length){
                    $("#selectAll01")[0].checked = true;
                }else{
                    $("#selectAll01")[0].checked = false;
                }
            }
        }
    }
