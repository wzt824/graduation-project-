<?php
	...
	//3）、传输数据（过桥）
	$sqlstr = "select * from favorite order by userPhone";
	...
	while($query_row){
		$str = $str.'{ "userPhone":"'.$query_row[0].'","proId":"'.$query_row[1].'","proImg":"'.$query_row[2].'","proName":"'.$query_row[3].'","proColor":"'.$query_row[4].'","proSize":"'.$query_row[5].'"}';
