<?php
	...
	//3）、传输数据（过桥）
	$sqlstr = "select * from shopcart order by userPhone";
	...
	while($query_row){
		$str = $str.'{ "userPhone":"'.$query_row[0].'","proId":"'.$query_row[1].'"
		,"proCount":"'.$query_row[2].'","proColor":"'.$query_row[3].'"
		,"proSize":"'.$query_row[4].'","proPrice":"'.$query_row[5].'"
		,"proPriceNum":"'.$query_row[6].'","proName":"'.$query_row[7].'"
		,"proImg":"'.$query_row[8].'"
		}';
